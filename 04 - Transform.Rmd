...
C. CHALLENGE (only if you have time): 
We may have some data anomolies in the data set in the form of missing data. Missing data can happen for many reasons, quite often because it's just not entered. Find all of the covid testing where the patient class (column `patient_class`) is not entered. In these cases the value will be NA. NA stands for "not available". It is the way that R codes missing values.  [Hint: See slide titled "Logical Tests"]
```{r Your Turn 3c}
...
How might you use arrange() to determine the highest and lowest Ct result in the dataset? (Hint: Try sorting the rows by the Ct result value) 
